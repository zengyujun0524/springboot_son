package com.example.springboot_son.utils.sms;
...
//		ss.send(0,"86","18814468057","123456为您的注册验证码，请于15分钟内填写。如非本人操作，请忽略本短信。", "", "");
...
//	}
	...
	public SmsSingleSenderResult send(int type, String nationCode, String phoneNumber, String msg, String extend,
		...
		JSONObject body = new JSONObject()
				...
				.put("type", type).put("msg", msg)
				.put("sig", SmsSenderUtil.calculateSignature(this.appkey, random, now, phoneNumber)).put("time", now)
		...
		HTTPRequest req = new HTTPRequest(HTTPMethod.POST, this.url).addHeader("Conetent-Type", "application/json")
				...
				.setConnectionTimeout(60 * 1000).setRequestTimeout(60 * 1000).setBody(body.toString());
	...
	public SmsSingleSenderResult sendWithParam(String nationCode, String phoneNumber, int templateId,
		...
		JSONObject body = new JSONObject()
				...
				.put("tpl_id", templateId).put("params", params).put("sign", sign).put("time", now)
				.put("extend", SmsSenderUtil.isNotEmpty(extend) ? extend : "")
				.put("ext", SmsSenderUtil.isNotEmpty(ext) ? ext : "");
	...
	public SmsSingleSenderResult sendWithParam(String nationCode, String phoneNumber, int templateId, String[] params,
		...
		return sendWithParam(nationCode, phoneNumber, templateId, new ArrayList<String>(Arrays.asList(params)), sign,
				extend, ext);
	}
}
